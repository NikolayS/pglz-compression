/* ----------
 * pg_lzcompress_conservative_ai.c -
 *
 *		"Conservative AI" variant — zero ratio impact, ~1.8× speedup.
 *
 *		Combines only the safe, ratio-preserving optimizations:
 *		1. Fibonacci multiplicative hash (fixes UB, better distribution)
 *		2. 4-byte memcmp fast-reject in match finding
 *		3. Chain length limit (256) for pathological input defense
 *
 *		NO skip-after-match (preserves compression ratio exactly).
 *		This is the variant most likely to be accepted by pgsql-hackers.
 *
 *		Additive to Vladimir's Steps 1-3 (inline functions, uint16 indexes,
 *		singly-linked list).
 *
...
/* ----------
 * Fibonacci hash — portable 4-byte read via memcpy, no UB
 * ----------
 */
 ...
 *
 * The 4-byte memcmp sacrifices rare 3-byte matches for consistent speed.
 * For tail bytes (<4 remaining), falls back to byte-by-byte.
 * ----------
 */
static inline int
