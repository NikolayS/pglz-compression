/* ----------
 * pg_lzcompress_combined_ai.c -
 *
 *		AI Alternative #3: Combined Fibonacci Hash + Skip After Match + 4-byte memcmp
 *
 *		Combines all three independent AI-explored optimizations:
 *		1. Fibonacci multiplicative hash (better distribution, shorter chains)
 *		2. Skip intermediate history adds after long matches (less hist_add work)
 *		3. 4-byte memcmp fast-reject in match finding (fewer comparisons)
 *
 *		This is the "AI best shot" variant to compare against Vladimir's approach.
 *
...
#define PGLZ_MAX_CHAIN			256		/* chain traversal limit */
...
/* ----------
 * Fibonacci hash — reads 4 bytes portably via memcpy
 * ----------
 */
...
/* ----------
 * Match finding with 4-byte memcmp fast-reject + chain limit
 * ----------
 */
static inline int
				...
				int *lenp, int *offp, int good_match, int good_drop, int mask)
{
	...
	while (hent != INVALID_ENTRY_PTR)
	{
		...
		/*
		 * 4-byte memcmp fast-reject: if the first 4 bytes don't match,
		 * skip immediately. We only use this when at least 4 bytes remain.
		 * For the tail (<4 bytes), fall back to byte-by-byte.
		 */
		if ((end - ip) >= 4)
		{
			if (memcmp(ip, hp, 4) != 0)
...
next_entry:
		...
		/* Chain length limit (defense against pathological hash collisions) */
		if (++chain_count >= PGLZ_MAX_CHAIN)
			break;
			  ...
			  const PGLZ_Strategy *strategy)
{
	...
	while (dp < dend)
	{
							...
							&match_off, good_match, good_drop, mask))
		{
			...
			/* Skip history adds after long matches */
